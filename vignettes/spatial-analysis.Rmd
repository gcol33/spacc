...
```{r sfar}
# Create fragmentation scenarios
set.seed(123)
n_scenarios <- 30
scenarios <- data.frame(
  richness = numeric(n_scenarios),
  area = numeric(n_scenarios),
  n_fragments = integer(n_scenarios)
)

for (i in seq_len(n_scenarios)) {
  n_frag <- sample(1:5, 1)
  n_sub <- sample(20:n_sites, 1)
  idx <- sample(n_sites, n_sub)
  scenarios$richness[i] <- sum(colSums(species[idx, , drop = FALSE]) > 0)
  scenarios$area[i] <- n_sub * 1.5  # proxy area
  scenarios$n_fragments[i] <- n_frag
}

sfar_fit <- sfar(scenarios$richness, scenarios$area, scenarios$n_fragments)
sfar_fit
```
...
effort <- rpois(n_scenarios, 10) + 1

sesars_fit <- sesars(scenarios$richness, scenarios$area, effort)
sesars_fit
```
