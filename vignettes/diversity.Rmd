...
phylogenetic or functional dispersion (Rao 1982). Both `spaccPhylo()` and
`spaccFunc()` expose it through `metric = "rao"`.
...
ordering. At each step the cumulative community is passed to a function
that returns a single number, so indices spacc does not implement
directly can still be tracked along the accumulation curve.
...
through `...`. The result inherits the `spacc` class, so `summary()`,
`plot()`, and `as.data.frame()` apply. Available orderings are `"knn"`,
`"kncn"`, `"random"`, `"radius"`, and `"collector"`.
