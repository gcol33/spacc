...
incoming and outgoing species are in trait space. Passing `traits` to
`spaccBeta()` scales the
...
assemblages. The phylogenetic analogue, `spaccBeta()` with a `tree` argument,
...
The continuous-$q$ profile extends to both paradigms. `diversityProfile()` with a
...
`"kncn"`, `"nn_walk"`, `"random"`, and `"collector"`. Because the index is an
