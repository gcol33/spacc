...
- `spacc()` is the main entry point
- All objects have `print()`, `summary()`, `plot()` methods
...
**Why?** Computing distances is O(n²). For large datasets, compute once and reuse:
...
| `plot.spacc()` | ggplot2 visualization |
| `as.data.frame.spacc()` | Convert to tidy format |
| `[.spacc()` | Subset seeds |
| `c.spacc()` | Combine objects |
...
All `plot()` methods return ggplot2 objects, so users can customize:
...
**Imports:**
- Rcpp, RcppParallel (C++ backend)
- ggplot2 (plotting)
- cli (progress bars, messages)

**Suggests:**
- vegan (for as_spacc.specaccum)
- mobr (for as_spacc.mob_out)
- sf (for spatial operations in subsample)
