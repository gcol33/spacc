...
**Benchmark target:** 10x faster than `dist()` for n > 1000
...
**Key optimization:** Pre-compute distance matrix once, reuse for all lookups
...
- Saturation point (where curve reaches X% of max)
...
**Features:**
- Mean line with CI ribbon
- Optional: individual seed curves
- Optional: saturation point marker
...
**Difference from kNN:** After each step, recalculate centroid of visited plots, find nearest unvisited to centroid (not to current plot)
